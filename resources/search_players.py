...
class SearchPlayers:
    ...
    def _set_attributes(self):
        ...
        self.attributes = {
            "info": ["short_name", "age", "club_name", "overall", "potential", "value_eur", "wage_eur",
                     "player_positons", "joined", "contract_valid_until", "pace", "shooting", "dribbling", "passing",
                     "defending", "physic"],
    ...
    def get_players(self, json_data: str) -> str:
        ...
        if req['ability1name']:
            search_df = search_df[search_df[req['ability1name']] >= int(req['ability1Value'])]

        if req['ability2Name']:
            search_df = search_df[search_df[req['ability2Name']] >= int(req['ability2Value'])]
...
if __name__ == "__main__":
    req_json = '{ "name":"" , "position":"" , "age":30, "ability1Name":"POT", "ability1Value":80 , ' \
               '"ability2Name":"OVA", "ability2Value":80}'

    search = SearchPlayers()
    print(search.get_suggestion("Ronal"))
    json = search.get_players(req_json)
    print(json)
